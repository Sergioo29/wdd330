...
function planetTemplate(planet) {
  return `<li>
    ...
    <p>Year:<span class="planet-year">${planet.orbital_period}</span></p>
    <p>Day:<span class="planet-day">${planet.rotation_period}</span></p>
...
function starshipTemplate(ship) {
  return `<li>
  <h4 class="planet-name">${ship.name}</h4>
  <p>Climate:<span class="planet-climate">${ship.passangers}</span></p>`
...
export async function renderFilmDetails(filmId, elementSelector) {
  ...
  try {
    ...
    addCategoryDetails(film.starshipTemplate, ".film__starships-list", starshipTemplate);
